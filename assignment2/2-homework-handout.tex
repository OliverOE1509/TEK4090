...
\begin{solution}
  ...
  \begin{center}
  \includegraphics[width=0.75\linewidth]{figures/Exc74.png}
  \end{center}

  The updated quaternion is found using $ \hat{q}_{k+1}^- = \bar{\Omega} \left( \hat{\omega}_k^+ \right) \hat{q}_k^+  $ 

  Discrete propagation of the covariance equation is $ P_{k+1}^- = \Phi_k P_k^+ \Phi_k^T + \gamma_k Q_k \gamma_k^T $ 

  Our task is to estimate the attitute of a spacecraft in orbit, using the Extended Kalman Filter (EKF). The spacecraft attitude, represented as a quaternion. 
