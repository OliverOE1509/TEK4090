...
\begin{solution}
  \begin{parts}
    ...
    \[
        \Psi = 
        \begin{bmatrix}
        I_n & 0 & \cdots & 0 & -B & 0 & \cdots & 0 \\
        -A & I_n & \cdots & 0 & 0 & -B & \cdots & 0 \\
        0 & -A & \ddots & 0 & 0 & 0 & \ddots & 0 \\
        \vdots & \vdots & \ddots & I_n & \vdots & \vdots & \ddots & -B \\
        0 & 0 & \cdots & -A & I_n & 0 & \cdots & 0
        \end{bmatrix}
        \in \mathbb{R}^{nT \times (nT + mT)}
    \]
    and 
    \[
        \Omega = \begin{bmatrix}
    ...
    The bounds are
    \begin{itemize}
      \item $ -f \leq z_1 \leq 5 $
      \item $ -\infty < z_2 < \infty $
      \item $ -1 \leq u \leq 1 $   
    \end{itemize}
    ...
    To avoid infeasibility when the reference pushed $ z_{1, k} $ near the boundaries $ \left[ -5, 5 \right]  $, the slack variables $ \sigma_k^u, \sigma_k^l \geq 0$ were introduced for each step $ k=1, ..., T $, which modifies the state constraints to 
    \[
        -5 - \sigma_k^l \leq z_{1,k} \leq 5 + \sigma _{ k} ^u
    \]
    These slack variables are appended to the decision vector 
    \[
        x = \begin{bmatrix}
            z\\ u\\ \sigma _{ } ^u\\ \sigma _{ } ^l 
        \end{bmatrix} 
    \]
    In the code, this required extending $ A_{ineq}, b_{ineq} $ to implement $ z_{1, k} - \sigma _{ k} ^u \leq 5, -z_{1, k} - \sigma _{ k} ^l \leq 5 $. A large quadratic penalty was added to $ \sigma _{ k} ^u, \sigma _{ k} ^l $ in the cost to keep violations small. 
        

    When we introduced the reference tracking $ y_k - y^d = C z_k - y_d $ we got that the expression $ \left( C z_k - y_k^d \right)^T Q_y \left( C z_k - y_k^d \right)   $  expanded to a quadratic, a linear and a constan term. We dropped the constant term from the function becuase its irrelevant to optimization, and got 
    \[
