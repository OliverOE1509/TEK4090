...
\textbf{Name of student: Oliver Ekeberg}
...
\begin{questions}
  ...
  \question[15] Suppose that $T:V\mapsto V$ is a linear map, and suppose that $S:V\mapsto V$ is an invertible linear map.
    \begin{parts}
      ...
      \begin{solution}
        If $ v $  is an eigenvector of $ T $, then $ S^{-1}(v) $ is an eigenvector of $ S^{-1}TS $   
      \end{solution}
...
\end{solution}
  ...
  \part Suppose $C(s)$ is given by 
    ...
    \begin{solution}
      ...
      So no, $ C(s) = \frac{1} {s}  $ does not stabilize the system because the characteristic equation $ 1+ P(s)C(s) =0$ has roots with positive real parts  
  ...
  \begin{solution}
    ...
    \] 
    If $ K=0 $ this gives that $ s=0 $ and $ s=-1/2 $ which gives marginal stability, and for $ K>0 $, it gives that s has no roots in the RHP and ensures stability. so $ K>0 $ gives stability, which is justified by the graph.
  ...
  \begin{solution}
    ...
    and our transfer function 

    \[
       \frac{Y(S)} {D(s)} = \frac{P(s)} {1 + P(s)C(s)} 
    \] 

    Substituting our plant and controller we get

    \begin{align*}
      \frac{Y(s)} {D(s)} &=  \frac{\frac{10} {s+1} } {1+ \frac{10K} {s(s+1)} } \\
      &= \frac{10} {s+1} * \frac{s(s+1)} {s(s+1) + 10K} \\
      &= \frac{10s} {s^2 +s + 10K}  
    \end{align*}
    ...
    \begin{align*}
      y_{ss} &= lim_{s \rightarrow 0} s * Y(s) \\
    \end{align*}

    We know that $ Y(s) = \frac{P(s)} {1 + P(s)C(s)} * D(s) = \left( \frac{10s} {s^2+s+10K} \right) * \left( \frac{100} {s} \right)  $ 
    
    so we get 

    \begin{align*}
      y_{ss} &= lim_{s \rightarrow 0} s * Y(s) \\
      &= lim_{s \rightarrow 0} s * \left( \frac{1000} {s^2+s+10K} \right)\\
      &= lim_{s \rightarrow 0} \frac{1000s} {s^2+s+10K} \\
      &= \frac{1000*0} {0+0+10K} \\
      &= \frac{0} {10K} \\
      &= 0   \\
    \end{align*}
    ...
    This result holds for any stable value $ K>0 $. It means that the feedback controller, which includes an integrator $ \frac{1} {S}  $ rejects the constant DC disturbance over time, driving the steady-state error to zero.  
...
\begin{solution}
  The figure for the Nyquist curve for $ P(s) $ 
...
\begin{solution}
  ...
  \[
     w(2-w^2) = 0
  \] 
  Which gives that $w = 0$ or $w = \pm \sqrt{2}$. Since two poles are located at the imaginary axis, we need to indent around them. If we then plug $ w = 0 $ to the real part of $ L(jw) = 0 $, then we find that $ K=4 $. And doing the same for $ w = \pm \sqrt{2} $, we find that $ K=0 $ 
...
\begin{solution}
  ...
  This has no finite solutions for any rational $ w $, only as w approaches $ \infty $ which means that the system is stable for all $ K>0 $ and $ \frac{-1} {K}  $ is then in the LHP. Therefore, the feedback system is stable for all real gains $ K>0 $.     
\end{solution}
