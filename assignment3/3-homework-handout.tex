\documentclass[12pt,a4paper, onecolumn]{exam}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{epstopdf}
\usepackage{listings}
\usepackage{changepage}  % Add this line with your other packages
\usepackage{booktabs}
\usepackage[lmargin=71pt, tmargin=1.2in]{geometry} %For centering solution box
\lhead{TEK4090 -- Introduction to Modern Control\\}
\rhead{Assignment 3\\}
% \chead{\hline} % Un-comment to draw line below header
\thispagestyle{empty}   %For removing header/footer from page 1

\usepackage{graphicx}
\graphicspath{{./figures/}}

\def\showsoln{1}
\printanswers

\begin{document}



\begingroup  
    \centering
    \LARGE TEK4090 -- Introduction to Modern Control\\
    \LARGE Assignment 2 v1.0\\[0.5em]
    \large Due: November 17 2025, 23:30\\
\endgroup
\rule{\textwidth}{0.4pt}
%\pointsdroppedatright   %Self-explanatory
%\printanswers


\renewcommand{\solutiontitle}{\noindent\textbf{Ans:}\enspace}   %Replace "Ans:" with starting keyword in solution box
\renewenvironment{solution}
  {\begin{adjustwidth}{-2cm}{-2cm}% Reduce left and right margins by 2cm each
   \par\noindent\solutiontitle\ignorespaces}
  {\end{adjustwidth}}


\begin{questions}

 \question[40] Consider the following model of two masses connected by a flexible structure, with a lateral force on one of the masses.
\begin{center}
  \includegraphics[width=0.5\linewidth]{./figures/flexStructure.png}
\end{center}
A dynamical system can be written as,
\begin{align}
  \left( M_{lump} + M_{rod} \right)
  \ddot U + 
  C_{rod} \dot U +
  K_{rod} U
  = 
  \begin{bmatrix}
    1 \\ 0
  \end{bmatrix}F,
\end{align}
where $U = \left[ x_1,x_2 \right]^T\in \mathbb{R}^2$ is the displacement of the two masses.
Given the density $\rho_r$ of the rod, the elastic modulus $E_r$ of the rod, the cross-sectional area $A_r$ of the rod, the length $L_r$ of the rod, and the masses $M_1$ and $M_2$, the system matrices can be written as,
\begin{align}
  M_{lump} &= 
  \begin{bmatrix}
    M_1 & 0 \\ 0 & M_1
  \end{bmatrix},~
  M_{rod} = 
  \dfrac{A_rL_r\rho_r}{6}
  \begin{bmatrix}
    2 & 1 \\ 1 & 2
  \end{bmatrix}\\
  C_{rod} &= \alpha K_{rod},~
  K_{rod} = 
  \dfrac{A_rE_r}{L_r}
  \begin{bmatrix}
    1 & -1 \\ -1 & 1
  \end{bmatrix}.
\end{align}
For simplicity, let
\begin{align}
  \dfrac{A_rL_r\rho_r}{6} = 1;~ \dfrac{A_rE_r}{L_r} = 1;~M_1=M_2=10;~\alpha = 0.1.
\end{align}
Finally, let the input be $F$ and the output be $x_2$.
\begin{parts}
  \part Obtain the state space model $\Sigma:= (A,B,C,D)$ of the system.
    Use Matlab to find the poles and zeros of the system.
  \begin{solution}
    We start off by setting $ M_{lump} + M_{rod} = M_t $
    then \[
        M_t = \begin{bmatrix}
            12&-1\\
            -1&12
        \end{bmatrix} 
    \]
    Furthermore, $ M_t^{-1}  = \frac{ 1 }{ 143 } \begin{bmatrix}
        12&-1\\
        -1&12
    \end{bmatrix} $ 

    Our state vector is $ x = \left[ x_1, x_2, \dot{x_1}, \dot{x_2} \right]^T  $ where 
    \begin{itemize}
      \item $ z_1 = x_1 $
      \item $ z_2 = x_2 $
      \item $ z_3 = \dot{x_1} $
      \item $ z_4 = \dot{x_2} $    
    \end{itemize} 
    We then get that $ \left[ \dot{z_1}, \dot{z_2}, \dot{z_3}, \dot{z_4} \right]^T = \left[ z_3, z_4, \ddot{x_1}, \ddot{x_2} \right]^T  $. Write now $ \dot{x} = Ax+BF $. Then 
    
    \[
        \begin{bmatrix}
            \ddot{x_1}\\
            \ddot{x_2}
        \end{bmatrix} = M_t^{-1} \left[ -C_{rod} \begin{bmatrix}
            z_3\\
            z_4
        \end{bmatrix} - K_{rod} \begin{bmatrix}
            z_1\\
            z_2
        \end{bmatrix} + \begin{bmatrix}
            F\\
            0
        \end{bmatrix}  \right]  
    \]

    I have to break this expression up. I first simplify $ -C_{rod} \begin{bmatrix}
        z_3\\
        z_4
    \end{bmatrix}  $
    \begin{align*}
      -C_{rod} \begin{bmatrix}
        z_3\\
        z_4
    \end{bmatrix} &= -0.1 \begin{bmatrix}
        1&-1\\
        -1&1
    \end{bmatrix} \begin{bmatrix}
        z_3\\
        z_4
    \end{bmatrix}  \\
    &= -0.1 \begin{bmatrix}
        z_3 - z_4\\
        z_4 - z_3
    \end{bmatrix} 
    \end{align*}

    Now for $ -K_{rod} \begin{bmatrix}
        z_1\\
        z_2
    \end{bmatrix}  $ 

    \begin{align*}
      -K_{rod} \begin{bmatrix}
        z_1\\
        z_2
    \end{bmatrix}  &= \begin{bmatrix}
        1&-1\\
        -1&1 
    \end{bmatrix} \begin{bmatrix}
        z_1\\
        z_2
    \end{bmatrix} \\
    &= \begin{bmatrix}
        z_1 - z_2\\
        z_2 - z_1
    \end{bmatrix}   
    \end{align*}
    
    And finally, $ B_0 F = \begin{bmatrix}
        F\\
        0    
      \end{bmatrix}  $ 
     
    We now get inside the bracket equation
    \[
        -C_{rod} - K_{rod} + B_0F
    \]
    which becomes

    \begin{align*}
      -0.1 \begin{bmatrix}
          z_3 - z_4\\
          z_4 - z_3
      \end{bmatrix} - \begin{bmatrix}
          z_1 - z_2\\
          z_2 - z_1
      \end{bmatrix} + \begin{bmatrix}
          F\\
          0
      \end{bmatrix}  = \begin{bmatrix}
          0.1z_4 - 0.1z_3 + z_2 - z_1 + F\\
          0.1z_3 - 0.1z_4 + z_1 - z_2 
      \end{bmatrix} = \psi
    \end{align*}
    
    Now we can solve $ M_t^{-1} $
    
    \begin{align*}
      M_t^{-1} \psi &= \frac{ 1 }{ 143 } \begin{bmatrix}
          12&-1\\
          -1&12
      \end{bmatrix} \begin{bmatrix}
          0.1z_4 - 0.1z_3 + z_2 - z_1 + F\\
          0.1z_3 - 0.1z_4 + z_1 - z_2 
      \end{bmatrix}\\
      &= \frac{ 1 }{ 143 } \begin{bmatrix}
          12 \left( 0.1z_4 - 0.1z_3 + z_2 - z_1 + F \right) - \left( 0.1z_3 - 0.1z_4 + z_1 - z_2 \right)  \\
          - \left( 0.1z_4 - 0.1z_3 + z_2 - z_1 + F \right) + 12 \left(0.1z_3 - 0.1z_4 + z_1 - z_2 \right)  
      \end{bmatrix}   \\
      &= \frac{ 1 }{ 143 } \begin{bmatrix}
          -12 z_1 + 12 z_2 - 1.2 z_3 + 1.2 z_4 -z_1 + z_2 -0.1z_3 + 0-1z_4 + 12 F\\
          z_1 - z_2 + 0.1z_3 - 0.1z_4 + 12z_1 - 12z_2 + 1.2z_3 - 1.2z_4 - F
      \end{bmatrix} \\
      &= \frac{ 1 }{ 143 } \begin{bmatrix}
          -13z_1 + 13z_2 - 1.3z_3 + 1.3 z_4 + 12F\\
          13z_1 - 13z_2 + 1.3z_3 - 1.3z_4 - F
      \end{bmatrix} 
    \end{align*}

    Since $ \dot{z_3} = \ddot{x_1}$ and $ \dot{z_4} = \ddot{x_2} $, we can find that 
    
    \[
        A = \begin{bmatrix}
            0&0&1&0\\
            0&0&0&1\\
            \frac{ -13 }{ 143 }& \frac{ 13 }{ 143 }& \frac{ -1.3 }{ 143 }& \frac{ 1.3 }{ 143 } \\
            \frac{ 13 }{ 143 }& \frac{ -13 }{ 143 }& \frac{ 1.3 }{ 143 }& \frac{ -1.3 }{ 143 }
        \end{bmatrix} 
    \]
    \[
        B = \begin{bmatrix}
            0\\
            0\\
            \frac{ 12 }{ 143 }\\
            \frac{ -1 }{ 143 }
        \end{bmatrix} 
    \]
    \[
        C = \begin{bmatrix}
            0&1&0&0
        \end{bmatrix} 
    \]
    \[
        D=0
    \]
    Since we have that $ y=x_2 = z_2 $, and our state vector is $ z = \begin{bmatrix}
        z_1\\z_2\\z_3\\z_4
    \end{bmatrix} = \begin{bmatrix}
        x_1\\x_2\\ \dot{x_1}\\ \dot{x_2}
    \end{bmatrix}  $  we can then derive the C matrix as 
    \[
        y = \begin{bmatrix}
            0&1&0&0
        \end{bmatrix}  \begin{bmatrix}
            x_1\\
            x_2\\
            \dot{x_1}\\
            \dot{x_2}
        \end{bmatrix} = x_2 
    \]
  \end{solution}
  
  \part Use Matlab to find the transfer function of the system, and verify the locations of the poles and zeros

  \begin{solution}
    We can find the transfer function and verify that the poles are the exact same as the eigenvalues for A
    \begin{lstlisting}[language = matlab]
    A = [0 0 1 0;
    0 0 0 1;
    -13/143 13/143 -1.3/143 1.3/143;
    13/143 -13/143 1.3/143 -1.3/143];
    B = [0;0;12;-1];
    C = [0 1 0 0];
    D = 0;

    O = obsv(A, C);


    sys = ss(A, B, C, D);
    poles = eig(A);
    zeros = tzero(sys);

    sys_tf = tf(sys);
    [num, den] = tfdata(sys_tf, 'v');
    roots_den = roots(den);

    disp(roots_den)
    disp(poles)

    \end{lstlisting}

    The output of the code is the following
    \begin{verbatim}
    -0.0091 + 0.4263i
    -0.0091 - 0.4263i
    -0.0000 + 0.0000i
    -0.0000 + 0.0000i

    -0.0091 + 0.4263i
    -0.0091 - 0.4263i
    -0.0000 + 0.0000i
    -0.0000 + 0.0000i
    \end{verbatim}
    
    We have that the transfer function is 
    \[
        \frac{ -0.006993s^2 + 0.000699s + 0.006993 }{ s^4 +. 0.01818s^3 + 0.1818s^2 }
    \]
    This is found with the following matlab commands

    \begin{verbatim}
      A = [0 0 1 0;
     0 0 0 1;
     -13/143 13/143 -1.3/143 1.3/143;
     13/143 -13/143 1.3/143 -1.3/143];
      B = [0; 0; 12/143; -1/143];
      C = [0 1 0 0];
      D = 0;

      sys = ss(A, B, C, D);
      [num, den] = tfdata(sys, 'v');
      num = round(num, 6);  den = round(den, 6);
      disp('Clean transfer function:');
      sys_tf_clean = tf(num, den);
    \end{verbatim}
    

    
  \end{solution}

  \part The desired output acceleration of the system is $\ddot y = A\sin(\omega t)$. Choose $A$ such that the output reaches 1 after one period $T=10s$, and set it so that the acceleration reaches zero after $T$. Finally, obtain and plot the desired velocity and position as a function of time from $t_0=0$ to $t_f=T$.

  \begin{solution}
  We solve first for the angular frequency $ \omega $. 
  \[
      \omega = \frac{ 2\pi }{ T } = \frac{ 2\pi }{ 10 } = 0.2\pi
  \]
  So the angular frequency is $ 0.2 \pi $ radians per second
  second

  We can find velocity by integrating the acceleration $ \ddot{y} = A \sin \left( \omega t \right)  $ 
  \begin{align*}
    \dot{y} &= \int_{}^{} A \sin \left( \omega t \right) dt   \\
    &= - \frac{ A }{ \omega } \cos \left( \omega t \right) + C_1
  \end{align*}

  And further for getting the position

  \begin{align*}
    y &= - \frac{ a }{ \omega } \int_{}^{}  \cos \left( \omega t \right) + C_1 dt \\
    &= \frac{ -A }{ \omega^2 } \sin \left( \omega t \right) + C_1t + C_2 
  \end{align*}
  
  Now we can apply the boundary conditions. The output $ y $ should reach 1 at the end of the period. And the acceleration should be 0 at the end of the period

  \begin{itemize}
    \item $ y(T) = 1 $
    \item $ \ddot{y}(T)= 0 $  
  \end{itemize}

  We found first that the angular velocity is $ \omega = 0.2 \pi $. Put this into $ \ddot{y}(T) = A \sin \left( \omega T \right)  $   where $ T=10 $ and we see this is satisfactory for any given A!

  If we are now to solve for the constants, we need initial conditions. Lets say that the speed and position is 0 at $ t=0 $ 

  \begin{align*}
    \dot{y}(0) &= - \frac{ A }{ \omega } \cos(0) + C_1\\
    &= 0\\
    - \frac{ A }{ \omega } &= C_1
  \end{align*}

  From $ y(0) = 0 $, we get that
  
  \begin{align*}
    y(0) &= - \frac{ A }{ {\omega^2} } \sin (0) + 0 \cdot C_1 + C_2 \\
    &= 0
    C_2 &= 0
  \end{align*}
  
  And we get our final position function to be
  \[
      y = - \frac{ A }{ \omega^2 } \sin \left( \omega t \right) + \frac{ A }{ \omega } t
  \]

  And now all remains is to solve for the final A that gives the final position value to be 1
  \begin{align*}
    y(T) &= - \frac{ A }{ \omega^2 } \sin \left( \omega T \right) + \frac{ A }{ \omega } T\\
    &= \frac{ A }{ \omega } T = 1\\
    A &= \frac{ \omega }{ T }\\
    &= 0.02 \pi
  \end{align*}
  
  Here is the plot that shows the trajectories of $ \ddot{y}, \dot{y}, y $ 

  \begin{center}
    \includegraphics[width = 0.8\linewidth]{./figures/exc1c_A3.pdf}
  \end{center}
  
  
   
  \end{solution}
  \part Choose your favourite feedback system to track the desired reference output (for example, LQR) and explain your design decisions.

  \begin{solution}
  I first want to mention that our system is nonminimum phase. Because when we initially push mass 1, the distance between mass 1 and mass 2 will initially decrese, and then start to increase above the threshold it started off at $ t_0 $. 
  
  For my design decisions, I use a lqr controller with the states $ x = \left[ 0, x_2, 0, \dot{x}_2 \right]  $.   \end{solution}

  \part Simulate and plot the output tracking of the system with this feedback (for example, using LSIM in Matlab). What happens to the tracking performance if $T$ is varied?

  \begin{solution}
  From the script thats in the code appendix, I get the following plot of the system, where I have included the desired trajectory we calculated in 1c), and the actual trajectory we get from the lqr controller

  \begin{center}
    \includegraphics[width = 1\linewidth]{exc_1d_A3.pdf}
  \end{center}

  From this, we see that the initial position first decreases, and then starts to follow the desired trajectory with a delay. This is due to the non minimum phase property of our system. If we increase T, we see that the state doesnt converge to our desired trajectory.\\

  As we vary T, we dont see any improvements. The tracking errors are linearly proportional to the time, and will only see an increase as time goes on.

  \begin{center}
    \includegraphics[width = 0.8\linewidth]{exc1e_A3.pdf}
  \end{center}

  The $ y_d $ trajectory continues to end at 1, because we just wary the angular velocity $ \omega $ 

  \end{solution}
  \part Find the internal dynamics of the system, then find the reference state trajectory $x_{ref}$ and the inverse input $u_{inv}$.
    Plot them, and describe how you found them.

  \begin{solution}
    We can first state that from devasia on feedforward control, then
    \begin{align*}
      \frac{ d^r }{ dt^r }y(t) &= CA^rx(t) + CA^{r-1}Bu(t)\\
      &= A_y x(t) + B_yu(t)
    \end{align*}

    where r is the difference in degrees of our transfer functions numerator and denominator. From 1b), we can obviously see that $ r=2 $ in our case ( $ r= $  difference. in degree of numerator and denominator of the transfer function). \\

    We want to find a transformation matrix T such that 
    \[
        z = Tx = \begin{bmatrix}
            \zeta\\
            \eta
        \end{bmatrix} = \begin{bmatrix}
            y\\
            \dot{y}\\
            \eta_1\\
            \eta_2
        \end{bmatrix} 
    \]
    From our system, we know that $$ y=x_2 =Cx = \begin{bmatrix}
        0&1&0&0
    \end{bmatrix} x $$ and $$ \dot{y} = CAx = \begin{bmatrix}
        0&1&0&0
    \end{bmatrix} A = \begin{bmatrix}
        0&0&0&1
    \end{bmatrix}  $$  
    Thus we know that $ T_{\zeta} = \begin{bmatrix}
        0&1&0&0\\
        0&0&0&1
    \end{bmatrix}  $. We are going to find $ Tx_{ref} = \begin{bmatrix}
        \zeta_d(t)\\
        \eta_{ref}(t)
    \end{bmatrix}   $ where we calculate $ \eta_{ref} $ from our internal dynamics. Thus, we need to find a $ T_{\eta} $ that makes
    \[
        T = \begin{bmatrix}
            T_{\zeta}\\
            T_{eta}
        \end{bmatrix} 
    \] invertible. An easy solution to this is to pick
    \[
        T_{eta} = \begin{bmatrix}
            1&0&0&0\\
            0&0&1&0
        \end{bmatrix} 
    \]
    Then T bascially has the same form as a permutation matrix, which automatically has the inversion property\\


    The internal dynamics are given as 
    \[
        \dot{\eta} = A_{inv} \eta + B_{inv} \mathcal{Y}_d
    \]
    where

    \begin{itemize}
      \item $ A_{inv} = T_{\eta} \left[ A - BB_y^{-1}A_y \right] T_r^{-1} $
      \item $ B_{inv} = \left[ T_{\eta} \left[ A - BB_y^{-1}A_y \right]T_l^{-1} : T_{\eta} BB_y^{-1} \right]  $  
      \item $ \mathcal{Y}_d = \begin{bmatrix}
          \zeta_d& \ddot{y}_d
      \end{bmatrix}^T = \begin{bmatrix}
          y_d&\dot{y}_d& \ddot{y}_d
      \end{bmatrix}   $ 
    \end{itemize}
     
    Since our system is non minimum phase, the $ 2 \times 2 $ matrix $ A_{inv} $ has a positive unstable eigenvalue. We need to decouple A. This allows us to solve for the stable part causally forward in time, and the unstable part backwards noncausal. We know that A is diagonalizable, we write $ A_{inv} = V D V^{-1} $. So in this new decoupled system, we get the following internal dynamics

    \[
        \frac{ d }{ dt } \begin{bmatrix}
            \eta_s\\ \eta_l
        \end{bmatrix}  = \begin{bmatrix}
            A_s&0\\
            0&A_l
        \end{bmatrix} \begin{bmatrix}
            \eta_s\\ \eta_l
        \end{bmatrix} + \begin{bmatrix}
            B_s\\ B_l
        \end{bmatrix} \mathcal{Y} 
    \]

    where $ A_s $  is the part that contains the stable eigenvaues, and $ A_l $ contains the unstable parts.    
  \end{solution}

  \part Simulate and plot the system with this feedforward input.

  \begin{solution}
  \begin{center}
    \includegraphics[width = 1 \linewidth]{exc1g_A3.pdf}
  \end{center}

  As you can see, we have a \textbf{\textit{big}} improvement from our feedforward controller. This can confirm that feedforward control largely improves performance of non-minimum phase systems, to the point where we have perfect control. We also start off with initial non-zero control effort, due to the anticipation. Amazing
  \end{solution}

  \part Add the feedforward and feedback inputs together, and simulate the results in Matlab. Which setup has the best tracking performance (i.e., feedback, feedforward, feedback + feedforward)?

  \begin{solution}

    I measure the best model with the following metric

  \begin{verbatim}
    tracking_error = y_desired - y_simulated
    error_metric = trapz(tracking_error)
  \end{verbatim}
  
  I will show a table with the metrics measured after discussing the results.


  First, I display the solution for the added control of the feedback and feedforward

  \[
      u_{total} = u_{fb} + u_{ff}
  \]
  This gives the following graph.

  \begin{center}
    \includegraphics[width = 0.8\linewidth]{exc1h_A3.pdf}
  \end{center}
  Very nice. Since I have already showed the plots with the controls gathered from feedback and feedforward from 1d) and 1f), I will just refer you back there. \\

  We see that the worst performing controller, is the one purely using feedback. This is because it will only regulate itself as it goes. But since our system has non-minimum phase, we need to be able to predict future behaviour and adjust our controls according to the predictions we have. This allows us to anticate future behaviour of our system.\\

  The table below has all the same environment assumption. All have the same time and the same desired output. The metrics take the integral of the difference between desired and actual trajectory. So the bigger the number, the more worse control

  \begin{table}[h]
  \centering
  \begin{tabular}{lcc}
  \hline
  \textbf{Control Method} & \textbf{Tracking Error} & \textbf{Control Effort} \\
  \hline
  FB & 101.3295 & 509.1809 \\
  FF & 0 & -635.3090 \\
  FB + FF & 34.8750 & -392.8959 \\
  \hline
  \end{tabular}
  \caption{Performance Comparison of Control Methods}
  \label{tab:control_comparison}
  \end{table}

  To conclude, the feedforward + feedback control is the most chill with a an impressive control effort. The purely feedforward control has achieved perfect control, but too high negative control effort, means that there is more effort in the opposite way we are pushing mass 1. So I would say that the feedback + feedforward control is the best, considering it has the smallest magnitude of all control efforts, as well as a reasonably low tracking error. Ultimately, it depends on your priorities.

  
  \end{solution}

  \part Using the feedback + feedforward controller in the previous part, modify the system slightly before simulation (for example, modify $M_1$ and $M_2$ by $\pm 1\%$) in order to simulate uncertainty in the model parameters. Which setup has the best tracking performance (i.e., feedback, feedforward, feedback + feedforward)?

  \begin{solution}
  The combined feedback + feedforward controller demonstrates the best performance of all controllers under uncertainty in the mass we initially push, with negligible performance across all tested cases. The cases I tried was the following.
  
  \begin{enumerate}
    \item \textbf{Nominal}: $M_1 = 10 \cdot 1$, $M_2 = 10 \cdot 1$
    \item \textbf{$M_1 + 1\%$}: $M_1 = 10 \cdot 1.01$, $M_2 = 10 \cdot 1$
    \item \textbf{$M_1 - 1\%$}: $M_1 = 10 \cdot 0.99$, $M_2 = 10 \cdot 1$
    \item \textbf{$M_2 + 1\%$}: $M_1 = 10 \cdot 1$, $M_2 = 10 \cdot 1.01$
    \item \textbf{$M_2 - 1\%$}: $M_1 = 10 \cdot 1$, $M_2 = 10 \cdot 0.99$
    \item \textbf{Both $+1\%$}: $M_1 = 10 \cdot 1.01$, $M_2 = 10 \cdot 1.01$
    \item \textbf{Both $-1\%$}: $M_1 = 10 \cdot 0.99$, $M_2 = 10 \cdot 0.99$
  \end{enumerate}


  Here is the solution after simulating with feedback + feedforward

  \begin{center}
    \includegraphics[width = 0.8\linewidth]{exc1i_FF_FB_A3.pdf}
  \end{center}

  here is the solution after simulating with purely feedback control. As you can see, the performance is still quite good. This makes sense, since the feedback control will regulate itself on the proper path with the gain matrix when we use LQR.

  \begin{center}
    \includegraphics[width = 0.8\linewidth]{exc1i_FB_A3.pdf}
  \end{center}

  But for Feedforward control, the performance seems to be horrible. This confirms that FF performs bad under uncertainty and noisy systems. This is likely due to the fact that it is solved backwards, and the control is then not consistent with two different simulations

  \begin{center}
    \includegraphics[width = 0.8\linewidth]{exc1i_FF_A3.pdf}
  \end{center}

  Even though feedback and feedforward + feedback looks awfully similar, you can see from this illustration of control effort between the two different controls, that they differ the most at the end of the time period

  \begin{center}
    \includegraphics[width = 0.8\linewidth]{exc1i_nominal_controlEffort.pdf}
  \end{center}
\end{solution}
\end{parts}




\question[20] 
Estimate the resistance value starting from a series of repeated current and voltage measurements.
Assume the true relationship
\[
u_0(t) = R_0\, i_0(t), \qquad t=1,2,\ldots,N,
\]
where $u_0,i_0$ are the exact (noise–free) voltage and current.

Two different voltmeters are used, producing two data sets: one with low noise and one with high noise.

\begin{itemize}
  \item \textbf{Data generation.} Generate an experiment with $N$ measurements where
  $i_0$ is uniformly distributed in $[-0.01,\,0.01]~\mathrm{A}$ and $R_0=1250~\Omega$.
  The current is measured without errors, while the voltage measured by the two voltmeters is disturbed by independent, zero-mean, normally distributed noise $n_u$:
  \[
  n_u \sim \mathcal{N}(0,\sigma_u^2), \qquad
  \sigma_u^2 =
  \begin{cases}
  1,  & \text{(good voltmeter)},\\
  16, & \text{(bad voltmeter)}.
  \end{cases}
  \]
  Thus, with the measured signals
  \[
  i(t)=i_0(t), \qquad
  u(t)=u_0(t)+n_u(t), \qquad t=1,2,\ldots,2N,
  \]
  the first $N$ samples come from the good voltmeter and the next $N$ from the bad one.
\end{itemize}
\begin{parts}
  \part[5] \textbf{Weighted least-squares (WLS).} Derive the WLS estimate $\hat R$ of $R$ by minimizing
  \[
  V_{\mathrm{WLS}} = \frac{1}{2N}\sum_{t=1}^{2N} \frac{\big(u(t)-R\,i(t)\big)^2}{w(t)},
  \]
  where $w(t)$ is the variance of the voltage noise corresponding to sample $t$.
  Also, derive the \textbf{unweighted} least squares estimator, which minimizes,
  \begin{align}
    V_{\mathrm{LS}} =  \frac{1}{2N}\sum_{t=1}^{2N} {\big(u(t)-R\,i(t)\big)^2}
  \end{align}

  \begin{solution}
    To find the optimal $ \hat{R}_{WLE} $, we differentiate with respect to R and set the expression equal to 0, to find the given R.
    
    \begin{align*}
      \frac{ \partial V_{WLS} }{ \partial R } &= 2 \cdot \frac{ 1 }{ 2N } \sum_{ t=1 }^{ 2N } \frac{ i \left( u - Ri \right)  }{ w } \\
      &= \frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } \frac{ i u  }{ w } - R\frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } \frac{ i^2  }{ w } \\
      \frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } \frac{ i u  }{ w } &= R\frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } \frac{ i^2 }{ w} \\
      \hat{R}_{WLS} &= \frac{ \sum_{ t=1 }^{ 2N } \frac{ i(t)u(t) }{ w(t) } }{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 }{ w(t) } }
    \end{align*}

    And for the LS method, without the weights added, we get

    \begin{align*}
      \frac{ \partial V_{LS} }{ \partial R } &= 2 \cdot \frac{ 1 }{ 2N } \sum_{ t=1 }^{ 2N } i \left( u - Ri \right) \\
      &= \frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } iu - R \frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } i^2  \\
      \frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } iu &= R \frac{ 1 }{ N } \sum_{ t=1 }^{ 2N } i^2 \\
      \hat{R}_{LS} &= \frac{ \sum_{ t=1 }^{ 2N } iu }{ \sum_{ t=1 }^{ 2N } i^2 }
    \end{align*}
    
    And the question should be answered
    
  \end{solution}

  \part[5] Set $N=100$. Compute an estimate $\hat{R}$ $10^5$ times, using different data each time (according to the Data Generation procedure above), for both the LS and WLS procedures.
    Plot the resulting values of $\hat R$ in two histograms with an appropriate number of bins, one histogram for WLS and one for LS.
    Discuss the results.

    \begin{solution}
      There should be no surprise that the WLS estimates of the Ohms provide the most accurate ones. This happens because we have two voltimeters, and we give the voltimeter we trust the most, the most weight. This weight is given as the variance of the voltage noise ordered by the voltimeters. 

      \begin{center}
        \includegraphics[width=1\linewidth]{Figure_6.pdf}
      \end{center}

      The estimation of $ \hat{R } $ over all the $ 10^5 $ experiments, leads to the figure above, which looks normally distributed, and indeed it obviously is, from the errors from the 2 voltimeters that are normally distributed

      \begin{center}
        \includegraphics[width=0.8\linewidth]{normality_WLS.pdf}
      \end{center}
      \begin{center}
        \includegraphics[width=0.8\linewidth]{normality_LS.pdf}
      \end{center}
      

      Basically, if the two arrays for the estimates, are closer to the straigt line with slope $ \frac{ 1 }{ 1320-1180 } = \frac{ 1 }{ 140 } $, the more normal it is distributed. The plots above confirms our belief.
    \end{solution}



  \part[10] Derive expressions for the variance of the WLS and the LS estimators. Compute the values of each based on the model parameters listed in the Data Generation procedure. Then, compute the sample variance of the estimates $\hat R$ in your experiments in part (a). Compare these to the expressions that you derived/computed. What would happen as the number of experiments increases from $10^5$?

  \begin{solution}
    To derive the variance of the estimators, we take the $ \hat{R}_{LS} $ and $ \hat{R}_{WLS} $ and set them as inputs to the function $ V(x) $
    
    \begin{align*}
      V(\hat{R}_{WLS}) &= V \left( \frac{ \sum_{ t=1 }^{ 2N } \frac{ i(t) u(t) }{ w(t) } }{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 }{ w(t) } } \right) \\
      &=V \left( \frac{ \sum_{ t=1 }^{ 2N } \frac{ i(t) \left( R_0i_0(t) + n_u(t) \right)  }{ w(t) } }{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 }{ w(t) } } \right) \\
      &= V \left( \frac{ \sum_{ t=1 }^{ 2N } \frac{ i(t)R_0i_0(t) }{ w(t) } }{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2  }{ w(t) } } + \frac{ \sum_{ t=1 }^{ 2N } \frac{ i(t) n_u(t) }{ w(t) } }{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 }{ w(t) } } \right) 
    \end{align*}

    
    Since $ i(t) = i_0(t) $ is the \textbf{\textit{true}} current, we can substitute that into the first fraction and get 

    \begin{align*}
      V(\hat{R}_{WLS} &= V(R_0) + \frac{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 V(n_u(t) }{ w(t)^2 } }{ \left( \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 }{ w(t) } \right)^2  }
    \end{align*}
    And since $ V(n_u(t)) = w(t) $, we get
    
    \begin{align*}
      V(\hat{R}_{WLS}) &= \frac{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 }{ w(t) } }{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^4 }{ w(t)^2 } }\\
      &= \frac{ 1 }{ \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 }{ w(t) } }
    \end{align*}
    
    Which is the final expression. Its important to note that for a linear equation $ a+bx $, then $ V(a+bx) = b^2V(x) $ where x is a random variable, which is why the $ R_0 $ term disappears
    
    Now for the derivation of $ \hat{R}_{LS} $ 

    \begin{align*}
      V(\hat{R}_{LS}) &=  V \left( \frac{ \sum_{ t=1 }^{ 2N } i(t)u(t) }{ \sum_{ t=1 }^{ 2N } i(t)^2 } \right) \\
      &= V \left( \sum_{ t=1 }^{ 2N } \frac{ i(t) \left( R_0i_0(t) + n_u(t) \right)  }{ i(t)^2 } \right) \\
      &= V \left( \sum_{ t=1 }^{ 2N } \frac{ i(t)^2 R_0 }{ i(t)^2 } + \sum_{ t=1 }^{ 2N } \frac{ i(t) n_u(t) }{ i(t)^2 } \right) \\
      &= V \left( R_0 + \sum_{ t=1 }^{ 2N } \frac{ i(t) n_u(t) }{ i(t)^2 } \right) \\
      &=  \frac{ \sum_{ t=1 }^{ 2N } i(t)^2 V(n_u(t)}{ \left( \sum_{ t=1 }^{ 2N } i(t)^2 \right)^2  } \\
      &= \frac{ \sum_{ t=1 }^{ 2N } i(t)^2 \sigma_u(t)^2 }{ \left( \sum_{ t=1 }^{ 2N } i(t)^2 \right)^2  }
    \end{align*}
    from the same reasoning that $ V(n_u(t))= w(t) $, and where 
    \[ 
        \sigma_u^2 = \begin{cases}
          1, \text{ good voltimeter}\\
          16, \text{ bad voltimeter}
        \end{cases}
    \]

    Now we can compare the theoretical with the empirical variances. Using the variances derived above, with $ N = 100 $, and generating $ \hat{R} $, $ 1e5 $ times I get:

    \begin{itemize}
      \item \textbf{\textit{Experimental variance}}
      \begin{itemize}
        \item WLS variance: $ 300.9280 $
        \item LS variance: $ 1.9484e+04 $  
      \end{itemize}
    \end{itemize}

    \begin{itemize}
      \item \textbf{\textit{theoretical variance}}
      \begin{itemize}
        \item WLS variance: $ 265.9457 $
        \item LS variance: $ 1.1347e+03 $  
      \end{itemize}
    \end{itemize}
    

    If we increase the number of experiments to $ 1e6 $, we will get more accurate results, where the experimental variance approaches the derived variance we found above. This is the law of large numbers coming in to play. 
    
  \end{solution}
\end{parts}

\question[20] \textbf{Least-Squares Identification of a One-Parameter Linear System.}
Consider the discrete-time system
\[
y_k = a\,y_{k-1} + b\,u_{k-1} + e_k, \qquad k = 2, \ldots, N,
\]
where
\begin{itemize}
  \item \(y_k\) is the measured output,
  \item \(u_k\) is the known input,
  \item \(e_k\) is a zero-mean white noise sequence,
  \item \(a,b\) are unknown parameters to be estimated.
\end{itemize}
Stack all samples \(k=2,\ldots,N\) to obtain
\[
\mathbf{y} = \Phi\,\theta + \mathbf{e},
\]
with
\begin{align}
\Phi =
\begin{bmatrix}
y_1 & u_1\\
y_2 & u_2\\
\vdots & \vdots\\
y_{N-1} & u_{N-1}
\end{bmatrix},
\qquad
\theta =
\begin{bmatrix}
a\\
b
\end{bmatrix},
\qquad
\mathbf{y} =
\begin{bmatrix}
y_2\\
y_3\\
\vdots\\
y_N
\end{bmatrix},
\qquad
\mathbf{e} =
\begin{bmatrix}
e_2\\
e_3\\
\vdots\\
e_N
\end{bmatrix}.
\label{eq:model}
\end{align}

\begin{parts}
\part[5]
By minimizing the sum of squared residuals
\[
V(a,b) = \sum_{k=2}^N \big(y_k - a\,y_{k-1} - b\,u_{k-1}\big)^2,
\]
show that the optimal parameters satisfy the \emph{normal equations}
\[
\Phi^\top \Phi\,\hat{\theta} = \Phi^\top \mathbf{y},
\]
leading to the closed-form least-squares solution
\begin{align}
  \boxed{\hat{\theta} = (\Phi^\top \Phi)^{-1}\Phi^\top \mathbf{y}.}\label{eq:ls}
\end{align}

\begin{solution}
  To derive the optimal parameters that satisfy the normal equations, we see first that from the exercise, we are given that we have dropped the error terms in the sum. And when we stack all the $ y_k, u_k, $ and all a, b that shall be  estimated, we get the sum thats given in the exercise. In quadratic form, this turns to

  \begin{align*}
    r &= y - \Phi \theta\\ 
    V(\theta) &= r^Tr\\ 
    &= \left( y - \Phi \theta \right) ^T \left( y - \Phi \theta \right) \\
    &= y^Ty - y^T \Phi \theta - \left( \Phi \theta \right)^T y + \left( \Phi \theta \right)^T \left( \Phi \theta \right) \\
    &= y^Ty - 2 \theta^T \Phi ^T y + \theta^T \Phi^T \Phi \theta
  \end{align*}
  


  
  And now we have to find the optimal parameters a, b that find the minimized sum of squared residuals

  \begin{align*}
    \frac{ \partial V(\theta) }{ \partial \theta } &= 0 - 2 \Phi ^T y + 2 \Phi^T \Phi \theta =0\\
    2\left( \Phi ^T \Phi \right)\hat{\theta} &=  2\Phi^T y \\
    \hat{\theta} &= \left( \Phi ^T \Phi \right) ^{-1} \Phi^T y
  \end{align*}
  This assumes of course that $ \Phi^T \Phi $ is invertible. I will get to this more in 3 c). But all in all, I think the exercise is solved.
  

\end{solution}

\part[10] Set $N=100$, $a=0.2$ and $b=-0.25$, and $e_k\sim \mathcal{N}(0,0.5)$, and consider the following input sequences:
\begin{itemize}
  \item a random input that is 1 half the time, and -1 half the time: 
    \begin{align}
      u_k \sim 
    \begin{cases}
      1 & {p=0.5}\\
      -1 & {p=-0.5}
    \end{cases}
    \end{align}
  \item a random uniform input:
    \begin{align}
      u_k \sim \textbf{randu[-1,1]}
    \end{align}
  \item a gaussian input with mean zero and variance 1:
    \begin{align}
      u_k \sim \mathcal{N}(0,1)
    \end{align}
\end{itemize}

For each input type, perform the following:
\begin{itemize}
  \item Generate $\mathbf{y}$ using Equation~\eqref{eq:model}
  \item Generate $\hat\theta$ using Equation~\eqref{eq:ls}
  \item Repeat this process $10^5$ times
\end{itemize}
Plot histograms for $\hat{a}$ and for $\hat{b}$ for the different input types on the same plot. Which input type yields the lowest variance estimate?

\begin{solution}
  I assume that for $ u_k $, that you mean that u is an $ n \times 1 $ vector, with $ -1 $ about half the time, and the other half is 1. Here is the histogram plots for the different a and b:

  \begin{center}
    \includegraphics[width = 1\linewidth]{./figures/figure_exc_3b_A3.pdf}
  \end{center}

  I will refer to the different control series generation processes like the following:

  \begin{itemize}
  \item \textbf{\textit{Method 1}}: a random input that is 1 half the time, and -1 half the time: 
    \begin{align}
      u_k \sim 
    \begin{cases}
      1 & {p=0.5}\\
      -1 & {p=-0.5}
    \end{cases}
    \end{align}
  \item \textbf{\textit{Method 2}}: a random uniform input:
    \begin{align}
      u_k \sim \textbf{randu[-1,1]}
    \end{align}
  \item \textbf{\textit{Method 3}}: a gaussian input with mean zero and variance 1:
    \begin{align}
      u_k \sim \mathcal{N}(0,1)
    \end{align}
\end{itemize}

  I also gather that the empirical variance of the $ \hat{a} $ and $ \hat{b} $ arrays vary between the method 1 and method 3. The following table displays the variance of said arrays, where column i is either the $ \hat{a} $ array or the $ \hat{b} $ array, and the column i is $ method_i $ 

  \begin{table}[h]
\centering
\begin{tabular}{lcc}
\hline
Input Type & Var(\^{a}) & Var(\^{b}) \\
\hline
Method 1 & 0.007833 & 0.002547 \\
Method 2 & 0.009062 & 0.007751 \\
Method 3 & 0.007848 & 0.002587 \\
\hline
\end{tabular}
\end{table}

From this, we see that method 2 is without a doubt the worst. It is relatively close between method 1 and method 3 having the lowest variance implication on the coefficient estimates $ \hat{a} $ and $ \hat{b} $.

It comes to the point where it differs between each time I run the code. 

\end{solution}

\part[5] Discussion
\begin{enumerate}
  \item Under what conditions is \(\Phi^\top\Phi\) invertible?
  \begin{solution}
      $ \Phi^T \Phi $ is invertible if and only if the following is satisfied; The columns of $ \Phi^T \Phi $ have full rank. The only case where this would not happen, was if the controls $ u_k = 0 \forall k$. This is unlikely in practice 
    \end{solution} 
  \item What happens if the input \(u_k\) is constant?
  \begin{solution}
      If one of the controls are constant, I dont see any much affection on the series y. The only thing I see happening is that the matrix $ \Phi $ has two columns that are nearly linearly dependent vectors, which means that they become ill-conditioned, which may result in higher variance. But if $ u_k=0 \forall k $ then the matrix $ \Phi^T \Phi $ is non-invertible. I mentioned this earlier. In this case, the estimates $ \hat{a} $ and $ \hat{b} $ are all zero. This is because the equation

      \[
          \hat{\theta} = \left( \Phi^T \Phi \right) ^{-1} \Phi^T y
      \]

      would not exist, due non-invertible property of the $ \Phi^T \Phi $ matrix
      
  \end{solution}
  \item How would correlated noise \(e_k\) affect the bias of the estimates? 
  \begin{solution}
    The linear model introduces bias to the estimates, and it will violate the assumptions of linear regression. This assumption is that $ e_k  $ is white noise, which is uncorrelated with the data. This will ultimately lead to wrong biased estimates 
  \end{solution}
\end{enumerate}


\end{parts}
\end{questions}
\end{document}