% ========== PART 1(i): PARAMETER UNCERTAINTY - COMBINED FF + FB ==========
...
denom_nominal = M_nominal^2 + 4*M_nominal + 3;
k_nominal = (M_nominal + 3) / denom_nominal;
c_nominal = 0.1 * (M_nominal + 3) / denom_nominal;

A_nominal = [0, 0, 1, 0;
            0, 0, 0, 1;
            -k_nominal, k_nominal, -c_nominal, c_nominal;
            k_nominal, -k_nominal, c_nominal, -c_nominal];

B_nominal = [0; 0; (M_nominal + 2)/denom_nominal; -1/denom_nominal];

% LQR design
Q = diag([1, 10, 0.1, 0.1]);  % State weighting: prioritize x2 position
R = 0.01;                      % Control effort weighting
K = lqr(A_nominal, B_nominal, Q, R);
...
for case_idx = 1:size(uncertainty_cases, 1)
    ...
    % ========== SIMULATION WITH MODEL MISMATCH - COMBINED FF + FB ==========
    x0 = [0; 0; 0; 0];
    x = x0;
    X_sim_actual = []; U_total_actual = []; Y_sim_actual = [];
    U_ff_actual = []; U_fb_actual = [];
    
    for k = 1:length(t)
        % Use NOMINAL reference
        x_ref_nominal = x_ref(:, k);
        
        % COMBINED FEEDFORWARD + FEEDBACK
        u_ff = u_inv(k);                    % Feedforward component
        u_fb = -K * (x - x_ref_nominal);    % Feedback component
        u_total = u_ff + u_fb;              % Combined control
        
        % Simulate with ACTUAL plant
        X_sim_actual = [X_sim_actual, x];
        U_total_actual = [U_total_actual, u_total];
        U_ff_actual = [U_ff_actual, u_ff];
        U_fb_actual = [U_fb_actual, u_fb];
    ...
    results(case_idx).U_ff = U_ff_actual;
    results(case_idx).U_fb = U_fb_actual;
...
sgtitle('Combined Feedforward + Feedback: Robustness to Parameter Uncertainty');


return
% Optional: Show control breakdown for nominal case
figure('Position', [100, 100, 800, 400]);
nominal_idx = 1;
plot(t, results(nominal_idx).U_ff, 'g-', 'LineWidth', 2, 'DisplayName', 'Feedforward'); hold on;
plot(t, results(nominal_idx).U_fb, 'r-', 'LineWidth', 2, 'DisplayName', 'Feedback');
plot(t, results(nominal_idx).U_total, 'b-', 'LineWidth', 2, 'DisplayName', 'Total Control');
title('Control Breakdown - Nominal Case');
xlabel('Time (s)'); ylabel('Control Input');
legend('Location', 'best'); grid on;